...
import {
  buildBalanceFournisseur,
  type LigneBalanceFournisseur,
    ...
    .map(
      (r) =>
        `<tr>
          ...
          <td style="padding:8px;border-bottom:1px solid #eee">${r.nomFournisseur}</td>
...
<style>
  ...
  <p>Factures achat uniquement (hors bons de commande)</p>
  <table>
    <thead><tr>
      <th>Date</th><th>ID</th><th>Nom Fournisseur</th><th>Total Factures</th><th>Total Payé</th><th>Total Solde</th>
    </tr></thead>
    <tbody>${body}</tbody>
  </table>
...
export default function BalanceFournisseurPage() {
  const router = useRouter();
  const [rows, setRows] = useState<LigneBalanceFournisseur[]>([]);
  const [ready, setReady] = useState(false);
  const [selectedId, setSelectedId] = useState<string | null>(null);

  function refresh() {
    setRows(buildBalanceFournisseur());
    setReady(true);
  }
  ...
  function onImprimer() {
    ...
    printBalanceRows(
      selected,
      selected.length === 1
        ? `Balance Fournisseur — ${selected[0].nomFournisseur}`
        : "Balance Fournisseur"
    );
  }
  ...
  return (
    <div className="space-y-3 px-4 pb-4 pt-1 md:px-6 md:pb-6 md:pt-2">
      <p className="text-center text-xs text-muted">
        Total Factures = factures achat uniquement (hors bons de commande)
      </p>
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  ...
                  >
                    Aucun fournisseur. Créez des fiches dans{" "}
                    <span className="font-semibold text-ink">Fiche Fournisseur</span>.
                  </td>
                </tr>
              ) : (
                rows.map((r, i) => (
