...
import { loadFacturesVente, type FactureVente } from "@/lib/factures-vente";
import {
  TYPES_REGLEMENT,
  calcSolde,
  formatDateFR,
  loadReglementsClient,
  nextReglementId,
  saveReglementsClient,
  todayISO,
  type ReglementClient,
} from "@/lib/reglements-client";
import type { TypeReglement } from "@/lib/clients";
...
const schema = z.object({
  ...
  dateEncaisse: z.string().min(1, "Date Encaiss obligatoire"),
...
function toFormValues(r: ReglementClient): FormValues {
  return {
    id: r.id,
    date: r.date,
    ...
    bnqRegl: r.bnqRegl || "",
    nomTire: r.nomTire || "",
    dateEncaisse: r.dateEncaisse,
    statut: r.statut ?? "Non",
  };
}

function reglementHtml(r: ReglementClient): string {
  const rows = [
    ...
    ["Bnq Règl", r.bnqRegl || "—"],
    ["Nom Tiré", r.nomTire || "—"],
    ["Date Encaiss", formatDateFR(r.dateEncaisse)],
    ["Statut", r.statut ?? "Non"],
  ]
    .map(
      ([k, v]) =>
...
<style>
  ...
  <h1>SpeedyPrint — Règlement Client</h1>
  <p>${r.refFacture} · ${r.nomClient}</p>
  <table>${rows}</table>
</body></html>`;
}

function printReglement(r: ReglementClient) {
...
function downloadPdf(r: ReglementClient) {
...
export default function ReglementClientPage() {
  const [list, setList] = useState<ReglementClient[]>([]);
  ...
  const {
    ...
    defaultValues: {
      ...
      bnqRegl: "",
      nomTire: "",
      dateEncaisse: todayISO(),
      statut: "Non",
    },
  });

  const watchDate = useWatch({ control, name: "date" });
  const watchDateEncaisse = useWatch({ control, name: "dateEncaisse" });
  ...
  const reglSansPiece = reglSansCheque;
  const pieceDisabled = readOnly || reglSansPiece;
  ...
  function openView(r: ReglementClient) {
    reset(toFormValues(r));
    setMode("view");
  }

  function openEdit(r: ReglementClient) {
    reset(toFormValues(r));
    setMode("edit");
  }
  ...
  function onFactureChange(factureId: string) {
    ...
    setValue("clientId", f?.clientId ?? "", { shouldValidate: true });
    setValue("nomClient", f?.nomClient ?? "", { shouldValidate: true });
    ...
    if (f?.echeance) {
      setValue("dateEncaisse", f.echeance);
    }
  }

  function onDelete(r: ReglementClient) {
  ...
  function onSubmit(values: FormValues) {
    ...
    const row: ReglementClient = {
      ...
      dateEncaisse: values.dateEncaisse,
        ...
        >
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-3">
            ...
            <div className="min-w-[165px] flex-[0.9]">
              <Field label="Date Encaiss" error={errors.dateEncaisse?.message}>
                <DateFrInput
                  value={watchDateEncaisse}
                  onChange={(iso) => setValue("dateEncaisse", iso, { shouldValidate: true })}
                  readOnly={readOnly}
          ...
          <table className="w-full min-w-[1100px] border-collapse text-center text-sm">
            <thead>
              <tr className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white">
                ...
                <th className="px-3 py-3.5 text-[12px] font-bold">Date Encaiss</th>
