...
const schema = z.object({
  date: z.string().min(1),
  id: z.string().min(1),
  nom: z.string().min(1, "Nom client obligatoire"),
  contact: z.string().min(1, "Contact obligatoire"),
...
function toFormValues(f: Client): FormValues {
  return {
    date: f.date,
    id: f.id,
    nom: f.nom,
...
<style>
  ...
  <h1>SpeedyPrint — Fiche Client</h1>
  <p>${f.nom} · ${f.id}</p>
  <table>${rows}</table>
...
export default function FicheClientPage() {
  ...
  }, [sorted, dateFrom, dateTo, searchClient]);
  ...
  function onDelete(f: Client) {
    const ok = window.confirm(
      `Supprimer le client « ${f.nom} » (${f.id}) ?`
    );
    if (!ok) return;
        ...
        >
          <div className="flex flex-wrap gap-x-3 gap-y-3">
            ...
            <div className="min-w-[240px] flex-[2.1]">
              <Field label="Nom Client" error={errors.nom?.message}>
                <Input
                  {...register("nom")}
            ...
            <div className="min-w-[90px] flex-[0.55]">
              <Field label="Bnq" error={errors.banque?.message}>
                <Input
                  {...register("banque")}
            ...
            <div className="min-w-[160px] flex-[1.35]">
        ...
        <div className="min-w-[220px] flex-[2]">
          <Field label="Client">
            <Input
              ...
              placeholder="Nom ou ID client…"
              className={inputShell}
            />
          </Field>
        </div>
        {(dateFrom || dateTo || searchClient) && (
          <Button
            type="button"
            variant="outline"
            onClick={() => {
